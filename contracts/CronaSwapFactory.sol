...
// Dependency file: contracts/swap/CronaSwapPair.sol
...
// Root file: contracts/swap/CronaSwapFactory.sol

pragma solidity =0.5.16;

// import 'contracts/swap/CronaSwapPair.sol';
// import 'contracts/interfaces/ICronaSwapFactory.sol';

contract CronaSwapFactory is ICronaSwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(CronaSwapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'CronaSwapFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'CronaSwapFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'CronaSwapFactory: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(CronaSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ICronaSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'CronaSwapFactory: FORBIDDEN');
        feeTo = _feeTo;
    }
